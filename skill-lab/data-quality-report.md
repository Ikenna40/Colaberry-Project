...
**Validation method**: Manual execution of the `data-quality-gate` procedure defined in `.claude/skills/data-quality-gate/SKILL.md`. The skill was **not** available for automatic invocation this session — it was created after the session started, and a direct invocation attempt returned `Unknown skill: data-quality-gate`. Restarting Claude Code (or starting a new session) is required for it to auto-trigger going forward. Source data was read only; `orders.csv` was not modified.

## Results

| Check | Evidence | Status | Recommended Action |
|---|---|---|---|
| Schema | Contract-required columns (`order_id`, `region`, `revenue`, `load_timestamp`) all present in the header | PASS | None |
| Freshness | 11/12 rows loaded 2026-08-03 (fresh); `ORD-1009` has `load_timestamp` = 2026-07-31T10:00:00Z — roughly 3 days old, exceeds the contract's 24-hour limit | **FAIL** | Re-extract/re-load the source batch behind `ORD-1009` before publishing |
| Expected volume | 12 rows present; contract minimum is 10 | PASS | None |
| Key uniqueness (`order_id`) | `ORD-1004` appears twice: row 5 (`order_date` 2026-08-02, `load_timestamp` 08:25) and row 10 (`order_date` 2026-08-03, `load_timestamp` 08:45) — same product/quantity/revenue, different dates | **FAIL** | Investigate the source system for ID collision or reuse; resolve to a single canonical row |
