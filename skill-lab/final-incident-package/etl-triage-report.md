...
- `skill-lab/final-incident-package/data-quality-report.md`, produced independently by the `data-quality-gate` skill from a data-quality check (not a pipeline run), had already flagged the same `ORD-1005` row for a blank `region` — two unrelated checks converging on the same fact.
...
4. **(Ruled out) Transient or infrastructure failure.** All 4 attempts show byte-identical error, row, and step, with no connectivity/timeout/credential errors anywhere in the log — the signature of a deterministic data problem, not a transient one (matches the "Retry that doesn't resolve" pattern: retries only help transient failures, and this failure never varied).

## Next Tests

(Diagnostic only — no code changes, no reruns)
